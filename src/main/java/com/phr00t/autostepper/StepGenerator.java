...
public class StepGenerator {
    ...
    private static int MAX_HOLD_BEAT_COUNT = 4;
    private static char EMPTY = '0', STEP = '1', HOLD = '2', STOP = '3', MINE = 'M';

    static Random rand = new Random();

    private static int getHoldCount() {
        int ret = 0;
        if (holding[0] > 0f) ret++;
        if (holding[1] > 0f) ret++;
        if (holding[2] > 0f) ret++;
    ...
    private static int getRandomHold() {
        int hc = getHoldCount();
        if (hc == 0) return -1;
    ...
    private static boolean sustainedFFT(
        float startTime,
        float len,
        float granularity,
        float timePerFFT,
        TFloatArrayList FFTMaxes,
        TFloatArrayList FFTAvg,
        float aboveAvg,
    ...
    public static String GenerateNotes(
        ...
        for (float t = timeOffset; t <= totalTime; t += timeGranularity) {
            ...
            if (t > 0f) {
                ...
                boolean sustained = sustainedFFT(
                    t,
                    0.75f,
                    timeGranularity,
                    timePerFFT,
                ...
                if (
                    nearKick &&
                    (nearSnare || nearEnergy) &&
                    timeIndex % 2 == 0 &&
                    steps > 0 &&
                    lastLine.contains("1") == false &&
                    lastLine.contains("2") == false &&
                    lastLine.contains("3") == false
                ) {

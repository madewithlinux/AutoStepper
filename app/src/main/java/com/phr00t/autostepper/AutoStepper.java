...
public class AutoStepper {
    ...
    public static void main(String[] args) {
        ...
        if (
            ...
            System.out.println(
                "Argument usage (all fields are optional):\n" +
    ...
    float getMostCommon(TFloatArrayList arr, float threshold, boolean closestToInteger) {
        ...
        for (int i = 0; i < values.size(); i++) {
            ...
            if (
                check.size() > longest ||
                check.size() == longest &&
                getDifferenceAverage(check) < getDifferenceAverage(longestList)
            ) {
                longest = check.size();
                longestList = check;
            }
    ...
    void analyzeUsingAudioRecordingStream(
        ...
        SMGenerator.AddNotes(
            smfile,
            SMGenerator.Beginner,
            StepGenerator.GenerateNotes(
                stepGranularity,
                HARDMODE ? 2 : 4,
                manyTimes,
                fewTimes,
